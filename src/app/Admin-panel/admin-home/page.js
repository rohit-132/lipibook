...
const AdminHome = () => {
  ...
  // ✅ Fixed routes to match "Admin-panel" (case-sensitive)
  const handleSignOut = () => router.push("/Admin-panel/admin-login");
  const handleHistory = () => router.push("/Admin-panel/admin-history");
  const handleStorage = () => router.push("/Admin-panel/admin-manage1");
  const handleConvert = () => {
    if (!selectedFile) {
      alert("Please upload a file first!");
      return;
    }
    router.push("/Admin-panel/translated-docs");
  };
